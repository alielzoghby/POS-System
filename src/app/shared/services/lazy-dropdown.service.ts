...
export class LazyDropdownService {
  ...
  getDropdownData(body: {
    type: string,
    params: any
  }): Observable<DropdownData[]> {
    return this.baseAPI.get(ApiConstant.GET_LOOKUP.replace('{type}', body.type), { params: filterNullEntity(body.params) }).pipe(
      map((res) => this.mapper.fromJson(DropdownData, res.data.lookups)
      )
    )
  }
}
